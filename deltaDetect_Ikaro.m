function [delta_parameters, delta] = deltaDetect_Ikaro(data_windowing, filtered_LFP1, fs, sd_threshold, plot_fig)
...
% Calculate the z-score considering the entire matrix
flattened_matrix = data_windowing.data_w(:);
% Calculate mean and standard deviation
mean_value = mean(flattened_matrix);
std_value = std(flattened_matrix);
...
%     delta(i,:)        = wavelet(data_windowing.data_w(i,:), fs);
    delta_zscore(i,:) = (delta(i,:) - mean_value) / std_value;
