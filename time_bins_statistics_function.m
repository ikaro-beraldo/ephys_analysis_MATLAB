%% Statistics function
function [results, normalized_results, anova_results, anova_rejected, correlation_results, filtered_correlation, results_names] = time_bins_statistics_function(directory,names,channel_selection,time_limit,b_length,bin_length)
...
for ii = 1:size(names,1)
    for jj = 1:size(names,2)
        ...
        %% MI Stable states REM

        filename = fullfile(directory,'MI_state_block',char(names{ii,jj}));
        load(filename,'MI_all','REM_idx')
        ...
        % Create a model vector (with the size == last REM epoch)
        model_vec = zeros(1,REM_idx(end));

        % LFP3 - 30 - 55
        model_vec(REM_idx(:,1)) = MI_all(1,:);
        results.MI_stable.REM.f_30_55(jj,ii,:) = time_bin_average(model_vec, bin_length, b_length, max_rec_length, REM_idx(:,1), true, [], []);

        % LFP3 - 65 - 90
        model_vec(REM_idx(:,1)) = MI_all(2,:);
        results.MI_stable.REM.f_65_90(jj,ii,:) = time_bin_average(model_vec, bin_length, b_length, max_rec_length, REM_idx(:,1), true, [], []);

        % LFP3 - 90 - 140
        model_vec(REM_idx(:,1)) = MI_all(3,:);
        results.MI_stable.REM.f_90_140(jj,ii,:) = time_bin_average(model_vec, bin_length, b_length, max_rec_length, REM_idx(:,1), true, [], []);


        % LFP3 - 150 - 170
        model_vec(REM_idx(:,1)) = MI_all(4,:);
        results.MI_stable.REM.f_150_170(jj,ii,:) = time_bin_average(model_vec, bin_length, b_length, max_rec_length, REM_idx(:,1), true, [], []);


        % LFP3 - 180 - 200
        model_vec(REM_idx(:,1)) = MI_all(5,:);
        results.MI_stable.REM.f_180_200(jj,ii,:) = time_bin_average(model_vec, bin_length, b_length, max_rec_length, REM_idx(:,1), true, [], []);


        % LFP3 - 200 - 250
        model_vec(REM_idx(:,1)) = MI_all(6,:);
        results.MI_stable.REM.f_200_250(jj,ii,:) = time_bin_average(model_vec, bin_length, b_length, max_rec_length, REM_idx(:,1), true, [], []);
        ...
        % Get the ripple accepted parameters
        load(filename,'ripple_accepted')
        load(filename2,'block')
        ripple_blocks = ripple_accepted(:,1);
        RDS_blocks.delta_ripple = block.delta_ripple_timestamps.delta(:,1);
        RDS_blocks.ripple_delta = block.ripple_delta_timestamps.ripple(:,1);
        RDS_blocks.delta_spindle = block.delta_spindle_timestamps.delta(:,1);
        RDS_blocks.ripple_delta_spindle = block.ripple_delta_spindle_timestamps.ripple(:,1);

%         if channel_selection(ii) == 2
%             load(filename,'ripple_result_block_2')
%             load(filename2,'block_2')
%             ripple_blocks = ripple_result_block_2(:,1);
...
%         end
        ...
        % Get the accepted ripples
        
        results.RDS.duration.ripple(jj,ii,:) = time_bin_average(duration.ripple(time_select), bin_length, b_length, max_rec_length, NREM, true, ripple_accepted(:,1), length(GMM.All_Sort));
        ripple_counts_non_normalized = time_bin_counting(ripple_blocks(ripple_blocks <= time_limit), bin_length, b_length, max_rec_length, n_epochs , NREM, true, false);
        results.RDS.duration.ripple_counts_higher_than_100ms(jj,ii,:) = time_bin_counting_condition(ripple_blocks(ripple_blocks <= time_limit), duration.ripple(time_select), bin_length, b_length, max_rec_length, n_epochs , NREM, true, false, 0.1,'high')...
            ./ ripple_counts_non_normalized;  
        results.RDS.RMS.ripple(jj,ii,:) = time_bin_average(RMS.ripple(time_select), bin_length, b_length, max_rec_length, NREM, true, ripple_accepted(:,1), length(GMM.All_Sort));
        results.RDS.PEAK.ripple(jj,ii,:) = time_bin_average(PEAK.ripple(time_select), bin_length, b_length, max_rec_length, NREM, true, ripple_accepted(:,1), length(GMM.All_Sort));
        results.RDS.FREQ.ripple(jj,ii,:) = time_bin_average(FREQ.ripple(time_select), bin_length, b_length, max_rec_length, NREM, true, ripple_accepted(:,1), length(GMM.All_Sort));


        %         if channel_selection(ii) == 2
        %             results.RDS.duration.ripple(jj,ii,:) = time_bin_average(duration.ripple_2(time_select_2), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_2(:,1), length(GMM.All_Sort));
%             results.RDS.RMS.ripple(jj,ii,:) = time_bin_average(RMS.ripple_2(time_select_2), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_2(:,1), length(GMM.All_Sort));
%             results.RDS.PEAK.ripple(jj,ii,:) = time_bin_average(PEAK.ripple_2(time_select_2), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_2(:,1), length(GMM.All_Sort));
%             results.RDS.FREQ.ripple(jj,ii,:) = time_bin_average(FREQ.ripple_2(time_select_2), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_2(:,1), length(GMM.All_Sort));
% 
%         elseif channel_selection(ii) == 3
%             results.RDS.duration.ripple(jj,ii,:) = time_bin_average(duration.ripple_3(time_select_3), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_3(:,1), length(GMM.All_Sort));
%             results.RDS.RMS.ripple(jj,ii,:) = time_bin_average(RMS.ripple_3(time_select_3), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_3(:,1), length(GMM.All_Sort));
%             results.RDS.PEAK.ripple(jj,ii,:) = time_bin_average(PEAK.ripple_3(time_select_3), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_3(:,1), length(GMM.All_Sort));
%             results.RDS.FREQ.ripple(jj,ii,:) = time_bin_average(FREQ.ripple_3(time_select_3), bin_length, b_length, max_rec_length, NREM, true, ripple_result_block_3(:,1), length(GMM.All_Sort));
%         end

        % Ripple Separated channels
        ...
        % Load files
        filename = fullfile(directory,char(names{ii,jj}),'architecture_segmented');
        load(filename)
        ...
        % Delta
        results.architecture.time_spent.NREM(jj,ii,:) = fix_length(Time_Spent_NREM,n_epochs);
        results.architecture.time_spent.REM(jj,ii,:) = fix_length(Time_Spent_REM,n_epochs);
        results.architecture.time_spent.WK(jj,ii,:) = fix_length(Time_Spent_AWAKE,n_epochs);
        % Theta
        results.architecture.n_bouts.NREM(jj,ii,:) = fix_length(Number_Bouts_NREM,n_epochs);
        results.architecture.n_bouts.REM(jj,ii,:) = fix_length(Number_Bouts_REM,n_epochs);
        results.architecture.n_bouts.WK(jj,ii,:) = fix_length(Number_Bouts_AWAKE,n_epochs);

        % Gamma
        results.architecture.d_bouts.NREM(jj,ii,:) = fix_length(Duration_Bouts_NREM,n_epochs);
        results.architecture.d_bouts.REM(jj,ii,:) = fix_length(Duration_Bouts_REM,n_epochs);
        results.architecture.d_bouts.WK(jj,ii,:) = fix_length(Duration_Bouts_AWAKE,n_epochs);
...
for ii = 1:length(results_names)
    for jj = 1:length(results_names)
        ...
        % Clear the empty (NaN) values
        nan_values = isnan(a) | isnan(b);
        a(nan_values) = [];
        b(nan_values) = [];
...
alfa = 0.05;
filtered = find([correlation_results{:,2}] < alfa & [correlation_results{:,2}] ~= 0 & [correlation_results{:,1}] < 0.999999999999 & ([correlation_results{:,1}] <= -0.6) | [correlation_results{:,1}] >= 0.6);
...
%% Function to sum the counting of something in time bins
function counting = time_bin_counting_condition(data_count,data_value, bin_length, block_length, max_l_expected, n_epochs, state_classification, consider_rest, normalize,condition_value,condition)
...
%  Define the data_counts that follows the condition
if strcmp(condition,'high')
    condition_limit = find(data_value >= condition_value);
...
for i = 1:n_bins
    ...
    % Finally count it
    counting(i) = length(find(ismember(data_count(condition_limit),bin_values)));
...
%% Function to transform a variable to a specific length (excluding elements or adding NaN)
function vector = fix_length(vector,expected_length)
l = length(vector);

% If the vector is bigger
if l > expected_length
    vector(expected_length+1:end) = [];
elseif l < expected_length
    vector = [vector; nan(expected_length - l, 1)];
end
