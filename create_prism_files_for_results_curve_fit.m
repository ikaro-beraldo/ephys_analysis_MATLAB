function create_prism_files_for_results_curve_fit(results_names, curve_fit_results, dir_to_save,flag_days)

if flag_days == 5
    % Get prism file model
    model_filename = "E:\Barnes Maze - Mestrad\dados matlab\PrismResults\ModelFile - CurveFit.pzfx";
elseif flag_days == 4
...
for r = 1:length(results_names)
    % Define the command
    command = sprintf("new_matrix = %s';",results_names{r});
    % Execute the command
    eval(command)
